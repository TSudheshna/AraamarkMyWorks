       ...
       <!--<span class="stationUL" ><xsl:value-of select="StationName" disable-output-escaping="yes"/></span> -->
        ...
        <xsl:element name="a">
          <xsl:attribute name="href">
            ./locationhome.aspx?locationid=<xsl:value-of select="locationid"/>&amp;pageid=20&amp;stationID=<xsl:value-of select="StationId"/>
          </xsl:attribute>
      ...
      <li>
        <xsl:if test="$bVerifyNutrition = 1">
          ...
          <xsl:element name="a">
            ...
            <xsl:text>        </xsl:text><xsl:value-of select="Calories" disable-output-escaping="yes"/>
          </xsl:element>
        </xsl:if>
